...
class AWhereAPI(object):
    ...
    def construct_response(self, potentialRatio, rainy, waterRequirements, resultGrowthStage, accGDD, pet, ppet):
        if (potentialRatio < 1) & (not rainy):
            if (self.LANG == 'en'):
                return 'Today\'s date is ' + self.END_DT + '. PET is ' + pet + 'and PPET is ' + ppet + '. So, the water requirements for your ' + self.CROP + ' crops are: ' + str(waterRequirements) + ' mm. Accumulated GDD is ' + accGDD + '. So, your crops\' growth stage is ' + resultGrowthStage + '.'
            elif (self.LANG == 'hi'):
                return 'आज की तारीख ' + self.END_DT + 'है। PET ' + pet + ' है और PPET ' + ppet + ' है। इसलिए, ' + self.CROP + ' की फसल के लिए आपकी पानी की जरूरतें ' + str(waterRequirements) + ' मिलीमीटर हैं। Accumulated GDD ' + accGDD + ' है। इसलिए,फसल वृद्धि अवस्था '+ resultGrowthStage + 'है।'
            elif (self.LANG == 'es'):
                return 'La fecha de hoy es ' + self.END_DT + '. PET es ' + pet + ' y PPET es ' + ppet + '. Asi que, las necesidades de agua de sus cultivos de ' + self.CROP + ' son de ' + str(waterRequirements) + ' milímetros. Accumulated GDD es ' + accGDD + '. Asi que, la etapa de crecimiento de los cultivos es ' + resultGrowthStage + '.'
            elif (self.LANG == 'fr'):
                return 'La date d\'aujourd\'hui est ' + self.END_DT + '. PET est ' + pet + ' et PPET est ' + ppet + '. Alors, les besoins en eau de vos cultures de ' + self.CROP + ' sont de ' + str(waterRequirements) + ' millimètres. Accumulated GDD est ' + accGDD + '. Alors, le stade de croissance de la culture est ' + resultGrowthStage + '.'
        else:
            if (self.LANG == 'en'):

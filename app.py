...
class AWhereAPI(object):
    ...
    def construct_response(self, potentialRatio, rainy, waterRequirements, resultGrowthStage, accGDD, pet, ppet):
        if (potentialRatio < 1) & (not rainy):
            if (self.LANG == 'en'):
                return 'Today\'s date is ' + self.END_DT + '. \nPET is ' + str(pet) + ' and PPET is ' + str(ppet) + '. So, the water requirements for your ' + self.CROP + ' crops are: ' + str(waterRequirements) + ' mm. \nAccumulated GDD is ' + str(accGDD) + '. So, your crops\' growth stage is ' + resultGrowthStage + '.'
            elif (self.LANG == 'hi'):
    ...
    def get_agronomic_url_today(self):
        ...
        return self.construct_response(potentialRatio, rainy, waterRequirements, resultGrowthStage, float("{0:.2f}".format(accGDD)), float("{0:.2f}".format(pet)), float("{0:.2f}".format(potentialRatio)))
